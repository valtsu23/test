...
## Version 2 changes
# Biggest change is updating screen only where it's required using display.update.
# This helps with cpu load and cpu temperature.
...
while loop:
    ...
    id = None
    # Reading message start and  message id
    if data == b'\xff':
        data = uart.read(1)
        if data == b'\xff':
            id = uart.read(2)
